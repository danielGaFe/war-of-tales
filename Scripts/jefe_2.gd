...
@onready var colision = $CollisionShape2D  # Cuerpo general (sin daño)
...
# Levitación
var amplitud := 300.0#Amplitud de la oscilación vertical
var velocidad := 5.0# Velocidad de oscilación
var base_y := 500.0# Posición Y de referencia
var time := 0.0# Tiempo acumulado para el calculo de la oscilación
...
func disparo():
	...
	var marcador = get_tree().root.get_node("NIVEL4")# Busca el nodo que lleva la puntuación
	bullet.connect("enemigo_muerto", Callable(marcador, "_on_enemigo_muerto"))# Conecta la señal de enemigo muerto con el marcador
	
func _process(delta):
	time += delta# Acumula tiempo
	position.y = base_y + sin(time * velocidad) * amplitud# Aplico la formula del movimiento senoidal
...
func recibir_golpe():
	if muerto:# Si ya está muerto, ignora el golpe
		return
	$grito.play()# Reproduce sonido de daño
	...
	if golpes_actuales >= golpes_necesarios:
		...
		var escena_portal = load("res://escenas/Portal_final.tscn")# Carga el portal que lleva al siguiente nivel
